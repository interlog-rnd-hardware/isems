...
const int BUFSIZE1 = 9;
char buf[BUFSIZE];
String myString; 
char myStringChar[BUFSIZE1];

void setup()
{
Serial.begin(9600);

strcpy(buf, "uhuy"); //buat ssid
eeprom_write_string(18, buf);

myString="1234567890"; //buat paass
myString.toCharArray(myStringChar, BUFSIZE); //convert string to char array
strcpy(buf, myStringChar);
eeprom_write_string(44, buf);
strcpy(buf, "30"); //buat buzzer
eeprom_write_string(65, buf);
strcpy(buf, "5");//buat kirim
eeprom_write_string(70, buf);

Serial.print("Reading string from address 18: "); 
eeprom_read_string(18, buf, BUFSIZE);
Serial.println(buf);
Serial.print("Reading string from address 44: "); 
eeprom_read_string(44, buf, BUFSIZE);
Serial.println(buf);
Serial.print("Reading string from address 65: "); 
eeprom_read_string(65, buf, BUFSIZE);
Serial.println(buf);
Serial.print("Reading string from address 70: ");
eeprom_read_string(70, buf, BUFSIZE);
Serial.println(buf);
}
